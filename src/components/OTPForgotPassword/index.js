...
const OTPForgotPassword = () => {
  ...
  const forgotData = useSelector(state => state.ForgotPasswordReducer);
  const otpData = useSelector(state => state.OTPForgotPassword);
  ...
  const VerifyOtp = () => {
    if (value.length == 4) {
      let data = {
        userId: forgotData.data.data.data._id,
        otp: value,
      };
